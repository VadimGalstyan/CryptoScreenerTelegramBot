...
namespace ScreenerTest.Services
{
    public class PriceTracker
    {
        ...
        public PriceTracker(decimal thresholdPercent, TimeSpan timeframe, Action<Alert> onAlert = null, Action<Alert> sendAlert = null)
        {
        ...
        public async Task AddPriceUpdate(PriceUpdate update,DatabaseService dbService)
        {
            ...
            if (Math.Abs(changePercent) >= _thresholdPercent)
            {
                ...
                if (!(await dbService.ContainingAlert(update.Pair)))
                {
